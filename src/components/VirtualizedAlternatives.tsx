            ...
            <div key={alt.id || index} className="absolute left-0 right-0 p-1" style={{ height: ROW_HEIGHT, top: index * ROW_HEIGHT }}>
              <div className="h-full p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-rose-700 bg-rose-50 px-2 py-0.5 rounded border border-rose-200">Hipótese Rejeitada #{index + 1}</span>
                  <span className="text-[11px] font-mono text-slate-500 font-semibold">Grau de Confiança: {Math.round(alt.confidence_score * 100)}%</span>
                </div>
                ...
                <div className="p-3 rounded-lg bg-white border border-slate-200 text-xs text-slate-700 shadow-xs">
                  <span className="text-amber-700 font-bold block mb-0.5">Fundamento Jurídico da Rejeição:</span>
